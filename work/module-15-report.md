...
## Script Metadata
- Filename: `scripts/validate_markdown.py`
- Language: Python
- Purpose: Walks the repository and validates every `.md` file for two consistency checks — (1) exactly one top-level `#` H1 title (counted outside fenced code blocks, with frontmatter-driven and `.github/` files exempt), and (2) every relative Markdown link (`[text](path)`) resolves to a file that exists on disk (skipping absolute URLs, anchors, and link syntax shown as inline-code documentation examples).
...
|-----------|-------------|---------|
| `--path` | Directory to scan for `.md` files (scans recursively, skipping `.git/`, `__pycache__/`, `node_modules/`, `.venv/`, `venv/`) | Repo root (parent of `scripts/`) |

## Test Run Output

Command: `py scripts/validate_markdown.py --path instructions`

```
OK create-status-report.agent.md
OK creating-instructions.agent.md
OK main.agent.md
OK manually-test-feature.agent.md
OK tag-extracted-items.agent.md
OK use-load_jira_pat.agent.md
OK use-load_notes.agent.md
OK use-merge_candidates.agent.md
OK validate-markdown.agent.md
OK verify-against-real-data.agent.md
OK write-readme-section.agent.md
```

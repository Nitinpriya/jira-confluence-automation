...
Source: https://github.com/codenjoyme/vibecoding-training/blob/main/instructions/creating-instructions.agent.md
...
- When creating skills, generate scripts that retrieve data from the file system and write it to other files during execution. This preserves tokens and minimizes hallucinations, so as a model, focus less on monitoring the data and rely more on the deterministic script developed within the skill.
...
- Add new file to the `./.github/prompts/` with name `to-[name].prompt.md` and the line we added to `main.agent.md` as reference to instruction file using platform-specific syntax as in example below:
